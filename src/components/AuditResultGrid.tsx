...
export default function AuditResultGrid({ results, onFlagClicked }: { results: any[], onFlagClicked: () => void }) {
  ...
  const data = {
    ...
    datasets: [
      {
        label: 'Impact Ratio',
        data: results.map(r => r.impact_ratio),
        backgroundColor: results.map(r => r.flagged ? 'rgba(239, 68, 68, 0.8)' : 'rgba(34, 197, 94, 0.8)'),
      }
    ]
  };
  ...
  const options = {
    responsive: true,
    scales: {
      y: { beginAtZero: true, max: 1.5, title: { display: true, text: 'Impact Ratio' } }
    },
    plugins: {
  ...
  return (
    <div className="p-6 bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10 shadow-2xl mt-6">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-white/10">
              <th className="pb-3 text-sm font-semibold text-gray-400 uppercase tracking-wider">Attribute</th>
          ...
          <tbody className="divide-y divide-white/5">
            {results.map((r, i) => (
              <tr key={i} className="text-white hover:bg-white/5 transition-colors cursor-pointer" onClick={() => { if(r.flagged) onFlagClicked(); }}>
                ...
                <td className="py-4 text-sm font-mono">{r.impact_ratio.toFixed(2)}</td>
                <td className="py-4">
                  {r.flagged ? (
                    <span className="inline-flex items-center gap-1.5 py-1 px-3 rounded-full text-xs font-medium bg-red-500/20 text-red-400 border border-red-500/20">
                      <div className="w-1.5 h-1.5 rounded-full bg-red-400"></div>Flagged (Bias)
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1.5 py-1 px-3 rounded-full text-xs font-medium bg-green-500/20 text-green-400 border border-green-500/20">
                      <div className="w-1.5 h-1.5 rounded-full bg-green-400"></div>Pass
                    </span>
                  )}
                </td>
              </tr>
      ...
      <div className="h-64 bg-black/30 p-4 rounded-xl">
