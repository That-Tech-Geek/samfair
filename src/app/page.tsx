...
export default function Home() {
  ...
  const [showEvaluatorMode, setShowEvaluatorMode] = useState(false);
      ...
      <nav className="border-b border-white/10 bg-black/20 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {user.isEvaluator && (
              <button 
                onClick={() => setShowEvaluatorMode(!showEvaluatorMode)}
                className={`ml-4 flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold transition-all ${showEvaluatorMode ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.5)]' : 'bg-white/10 hover:bg-white/20 text-blue-400 border border-blue-500/30'}`}
              >
                <Presentation className="w-4 h-4" />
                {showEvaluatorMode ? "Exit Evaluator Mode" : "Evaluator Mode"}
              </button>
            )}
          </div>
